...
class Resize(object):
    ...
    def constrain_to_multiple_of(self, x: float, min_val: int = 0, max_val: int = None) -> int:
...
def apply_min_size(sample: Dict[str, Any], size: Tuple[int, int],
                    image_interpolation_method: int = cv2.INTER_AREA) -> Tuple[int, int]:
...
def load_frame(frame: np.ndarray) -> Tuple[np.ndarray, Tuple[int, int]]:
    image = frame.copy()  # H, W, C
    orig_shape = image.shape[:2]
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB) / 255.0
    image = transform({"image": image})["image"]  # C, H, W
    image = image[None]  # B, C, H, W
